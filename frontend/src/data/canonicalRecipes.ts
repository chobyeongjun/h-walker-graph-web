...
// must be explicitly requested. Continuous signals (GRF waveform, joint
// angle ensemble) stay as graphs because their shape matters.
//
...
const IMU_RECIPES: CanonicalRecipe[] = [
  ...
  { id: 'joint_avg',  label: 'Joint angle · mean ± SD per cycle (per channel)',
                                                                       default: true, type: 'graph',   graph: 'imu_avg',
    hint: 'Stride-averaged joint angle per channel across the 0–100% gait cycle. The canonical kinematic figure.' },
  { id: 'rom_bar',    label: 'ROM bar (joint × plane × side)',        default: true, type: 'graph',   graph: 'rom_bar',
