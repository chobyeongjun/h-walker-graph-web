...
function graphBody(c){
  ...
  return `
    <div class="graph-toolbar">
      ...
      ${P ? `<span class="gt-meta">${P.width_mm}mm · ${P.body_pt}pt · ${P.dpi}dpi</span>` : ''}
      <div class="gt-export">
        <button class="gt-btn" data-graph-act="export">Export ▾</button>
        <div class="gt-menu" hidden>
          ${(P ? P.export_formats : ['PDF','SVG','PNG']).map(f=>
            `<button class="gt-mitem" data-graph-act="export-do" data-fmt="${f}">${f}</button>`).join('')}
        </div>
      </div>
    </div>
...
function wireCell(el, c, i){
  ...
  el.querySelector('[data-graph-act="preset"]')?.addEventListener('change',e=>{
    ...
    toast(`Preset → ${P.name} · ${P.width_mm}mm · ${P.body_pt}pt`);
  });
  ...
  el.querySelectorAll('[data-graph-act="export-do"]').forEach(b=>{
    b.addEventListener('click',()=>{
      ...
      toast(`Exporting ${fmt} @ ${P.dpi}dpi · ${P.width_mm}mm`);
    });
  });
