...
function result = coreAnalysis(T, label, filepath, syncId, syncWindow)
    ...
    for si = 1:2
        ...
        % Threshold-based onset-release (DesForce > 1 N segments)
        try
            ft.onsetRelease = hwalker.force.onsetReleaseRMSE(T, side, ...
                'Threshold', 1.0, 'Restrict', 'all', 'SegMinDurMs', 100);
        catch ME
            warning('hwalker:analyzeFile:onsetReleaseFail', '%s', ME.message);
        end
        % GCP-based onset-release RMSE — paper-grade window 55-85% GC
        try
            ft.onsetReleaseGCP = hwalker.force.onsetReleaseRMSE_GCP( ...
                T, side, hsIdx, validMask, 'OnsetPct', 55, 'ReleasePct', 85);
        catch ME
            warning('hwalker:analyzeFile:onsetReleaseGCPFail', '%s', ME.message);
        end
        result.([sf 'Force']) = ft;
        if ft.rmse > 0
            extra = '';
            if isfield(ft, 'onsetReleaseGCP') && ft.onsetReleaseGCP.nStrides > 0
                gcp = ft.onsetReleaseGCP;
                extra = sprintf('  | GCP[%g-%g%%]: RMSE=%.2f N peak_des=%.1f peak_act=%.1f lat=%.0fms', ...
                    gcp.window_pct(1), gcp.window_pct(2), gcp.rmse_overall_N, ...
                    nanmean1(gcp.peak_des_per_stride_N), ...
                    nanmean1(gcp.peak_act_per_stride_N), ...
                    nanmean1(gcp.latency_onset_ms));
            end
            fprintf('    %s force: RMSE=%.2f N (full), peak=%.2f N%s\n', ...
                side, ft.rmse, ft.peakError, extra);
        end
...
function ft = emptyForce()
    ...
    ft.onsetReleaseGCP = struct('window_pct', [55 85], 'nStrides', 0, ...
        'rmse_overall_N', NaN, 'mae_overall_N', NaN, ...
        'rmse_per_stride_N', [], 'mae_per_stride_N', [], ...
        'peak_des_per_stride_N', [], 'peak_act_per_stride_N', [], ...
        'latency_onset_ms', []);
end
